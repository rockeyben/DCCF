...
class IHModelWithBackboneMhead(nn.Module):
    def __init__(
        self,
        model, backbone,
        downsize_backbone_input=False,
        ...
        backbone_conv1_channels : int
            If mask_fusion is 'sum', define the number of channels for the convolution applied to the mask.
        """
        super(IHModelWithBackboneMhead, self).__init__()
        self.downsize_backbone_input = downsize_backbone_input
        self.mask_fusion = mask_fusion
    ...
    def forward(self, image, hsv, mask):
        """
        Forward the backbone model and then the base model, supported by the backbone feature maps.
        Return model predictions.
        ...
        output = self.model(image, hsv, mask, backbone_features)
        return output



class IHModelWithBackboneUpsample(nn.Module):
    def __init__(
        self,
        model, backbone,
        downsize_backbone_input=False,
        ...
        super(IHModelWithBackboneUpsample, self).__init__()
        self.downsize_backbone_input = downsize_backbone_input
        self.mask_fusion = mask_fusion
    ...
    def forward(self, image, image_fullres, mask, test=False, rois=None):
        ...
        output = self.model(image, image_fullres, mask, backbone_features, test, rois)
        return output