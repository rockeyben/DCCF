...
class UpsampleHTrainer(object):
    def __init__(self, model, cfg, model_cfg, loss_cfg,
        ...
        if cfg.multi_gpu:
            ...
            #self.net = BalancedDataParallel(2, self.net, dim=0)
    ...
    def training(self, epoch):
        ...
        for i, batch_data in enumerate(tbar):
            ...
            tbar.set_description(f'Epoch {epoch}, lowres {lowres_loss/(i+1):.5f}, fullres {fullres_loss/(i+1):.5f}')
            #for metric in self.train_metrics:
            #    metric.log_states(self.sw, f'{log_prefix}Metrics/{metric.name}', global_step)


        logger.info(f'Epoch {epoch}, lowres {lowres_loss/(len(self.train_data)):.5f}, fullres {fullres_loss/(len(self.train_data)):.5f}')
    ...
    def validation(self, epoch):
        ...
        for metric in self.val_metrics:
            self.sw.add_scalar(tag=f'{log_prefix}Metrics/epoch_{metric.name}', value=0,
                               global_step=epoch, disable_avg=True)
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            #print(torch.min(batch_data['images']), torch.max(batch_data['images']))
            #print(torch.min(batch_data['masks']), torch.max(batch_data['masks']))
            ...
            output = self.net(images, images_fullres, masks)
            #print(prof.table())
            loss = 0.0
