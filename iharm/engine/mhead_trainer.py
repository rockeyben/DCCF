...
class MheadHTrainer(object):
    ...
    def training(self, epoch):
        ...
        for i, batch_data in enumerate(tbar):
            ...
            pixel_loss += np.array(losses_logging.get('pixel_loss')).mean()
            #h_loss += np.array(losses_logging.get('h_loss')).mean()
            s_loss += np.array(losses_logging.get('s_loss')).mean()
            v_loss += np.array(losses_logging.get('v_loss')).mean()
            ...
            tbar.set_description(f'Epoch {epoch}, training loss {train_loss/(i+1):.6f} pixel loss {pixel_loss/(i+1):.6f} h loss {h_loss/(i+1):.6f} s loss {s_loss/(i+1):.6f} v loss {v_loss/(i+1):.6f}' )
            for metric in self.train_metrics:
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            images_h, images_s, images_v = batch_data['images_h'], batch_data['images_s'], batch_data['images_v']
            hsv = {
                'h' : images_h,
                's' : images_s,
                'v' : images_v
            }
            output = self.net(images, hsv, masks)

            loss = 0.0
            loss = self.add_loss('pixel_loss', loss, losses_logging, validation, output, batch_data)
            loss = self.add_loss('h_loss', loss, losses_logging, validation, output, batch_data)
            loss = self.add_loss('s_loss', loss, losses_logging, validation, output, batch_data)
            loss = self.add_loss('v_loss', loss, losses_logging, validation, output, batch_data)
    ...
    def add_loss(self, loss_name, total_loss, losses_logging, validation, net_outputs, batch_data):
        ...
        if loss_weight > 0.0:
            ...
            losses_logging[loss_name].append(loss_weight * loss.item())
