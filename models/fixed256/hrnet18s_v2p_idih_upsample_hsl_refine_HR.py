...
from iharm.engine.upsample_hsl_mc_trainer import UpsampleHSLMcTrainer
from iharm.mconfigs import BMCONFIGS
from iharm.model import initializer
from iharm.model.backboned import HRNetIHModelHSL
...
def init_model(cfg):
    ...
    ccfg = BMCONFIGS['improved_dih256_hsl_refine_HR']
    ccfg['params']['up_size'] = model_cfg.crop_size
    model = HRNetIHModelHSL(ccfg, pyramid_channels=256)
...
def train(model, cfg, model_cfg, start_epoch=0):
    ...
    wl = 1.0
    blur = False
    use_hr = BMCONFIGS['improved_dih256_hsl_refine_HR']['params']['use_hr'] # True
    ...
    loss_cfg.stage3_rgb_loss_weight = 1.0 * 0.01

    loss_cfg.stage3_rgb_loss_fullres = MaskWeightedMSE(min_area=100, pred_name='stage3_output_fullres',
            gt_image_name='target_images_fullres', gt_mask_name='masks')
    loss_cfg.stage3_rgb_loss_fullres_weight = 1.0 * 0.01
    ...
    lr_scheduler = partial(torch.optim.lr_scheduler.MultiStepLR,
                           milestones=[60, 75], gamma=0.1)
    trainer = UpsampleHSLMcTrainer(
